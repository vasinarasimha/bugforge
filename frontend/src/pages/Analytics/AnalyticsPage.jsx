...
export default function AnalyticsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await getAnalyticsOverview(projId, days)
      setData(res.data)
    } catch (err) {
  ...
  const createdArea = trends.length > 0
    ? `M ${getX(0)},${chartH - padY} ` + trends.map((t, i) => `L ${getX(i)},${getY(t.created_count)}`).join(' ') + ` L ${getX(trends.length - 1)},${chartH - padY} Z`
    : ''
    ...
    <div className="analytics-page">
      ...
      <section className="analytics-header">
        <div>
          <span className="analytics-eyebrow">
            <i className="bi bi-graph-up-arrow" /> Real-time Database Telemetry
          </span>
          <h2>Defect Analytics & Insights</h2>
          <p>Comprehensive defect lifecycle metrics, workload distributions, and resolution performance.</p>
        </div>
      ...
      <section className="analytics-kpi-grid">
        <div className="analytics-kpi-card tone-primary">
          <div className="kpi-icon-wrap"><i className="bi bi-bug-fill" /></div>
          <div className="kpi-content">
            <span className="kpi-label">Total Defects</span>
            <span className="kpi-value">{loading ? '—' : kpis?.total_defects ?? 0}</span>
            <span className="kpi-sub">Total logged defects</span>
          </div>
        </div>

        <div className="analytics-kpi-card tone-warning">
          <div className="kpi-icon-wrap"><i className="bi bi-exclamation-circle" /></div>
          <div className="kpi-content">
            <span className="kpi-label">Open Defects</span>
            <span className="kpi-value">{loading ? '—' : kpis?.open_defects ?? 0}</span>
            <span className="kpi-sub">Awaiting investigation</span>
          </div>
        </div>

        <div className="analytics-kpi-card tone-purple">
          <div className="kpi-icon-wrap"><i className="bi bi-arrow-repeat" /></div>
          <div className="kpi-content">
            <span className="kpi-label">In Progress</span>
            <span className="kpi-value">{loading ? '—' : kpis?.in_progress_defects ?? 0}</span>
            <span className="kpi-sub">Actively being fixed</span>
          </div>
        </div>

        <div className="analytics-kpi-card tone-success">
          <div className="kpi-icon-wrap"><i className="bi bi-check2-circle" /></div>
          <div className="kpi-content">
            <span className="kpi-label">Resolved</span>
            <span className="kpi-value">{loading ? '—' : kpis?.resolved_defects ?? 0}</span>
            <span className="kpi-sub">Fix verified & logged</span>
          </div>
        </div>

        <div className="analytics-kpi-card tone-slate">
          <div className="kpi-icon-wrap"><i className="bi bi-archive" /></div>
          <div className="kpi-content">
            <span className="kpi-label">Closed</span>
            <span className="kpi-value">{loading ? '—' : kpis?.closed_defects ?? 0}</span>
            <span className="kpi-sub">Lifecycle complete</span>
          </div>
        </div>

        <div className="analytics-kpi-card tone-indigo">
          <div className="kpi-icon-wrap"><i className="bi bi-stopwatch" /></div>
          <div className="kpi-content">
            <span className="kpi-label">Avg Resolution Time</span>
            <span className="kpi-value kpi-value-sm">{loading ? '—' : (resolutionMetrics?.formatted || 'No data')}</span>
            <span className="kpi-sub">
              {resolutionMetrics?.sample_size ? `From ${resolutionMetrics.sample_size} resolved defects` : 'No resolution data yet'}
            </span>
          </div>
        </div>
      </section>
        ...
        <div className="analytics-panel">
          <div className="panel-header">
            ...
            <div className="trend-legend">
              <span className="legend-item"><span className="legend-dot created" /> Created</span>
          ...
          <div className="chart-container">
            {trends.length === 0 || totalDefects === 0 ? (
              ...
              <div style={{ width: '100%', position: 'relative' }}>
                <svg viewBox={`0 0 ${chartW} ${chartH}`} className="trend-svg" style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
                  <defs>
                  ...
                  {[0, 0.25, 0.5, 0.75, 1].map((p, idx) => {
                    ...
                    return (
                      <g key={idx}>
                        <line x1={padX} y1={y} x2={chartW - padX} y2={y} stroke="rgba(148,163,184,0.18)" strokeDasharray="3 3" />
                  ...
                  {/* Lines */}
                  {createdPoints && <polyline fill="none" stroke="#3b82f6" strokeWidth="2.5" points={createdPoints} strokeLinecap="round" />}
                  {resolvedPoints && <polyline fill="none" stroke="#10b981" strokeWidth="2.5" points={resolvedPoints} strokeLinecap="round" />}
                  ...
                  {trends.map((t, idx) => {
                    ...
                    return (
                      <g key={idx} onMouseEnter={() => setHoveredTrendIndex(idx)} onMouseLeave={() => setHoveredTrendIndex(null)}>
                        {/* Hover bar */}
                        <line x1={cx} y1={padY} x2={cx} y2={chartH - padY} stroke={isHovered ? 'rgba(59,130,246,0.3)' : 'transparent'} strokeWidth="12" />
                        <circle cx={cx} cy={cyCreated} r={isHovered ? 5 : 3} fill="#3b82f6" stroke="#fff" strokeWidth="1.5" />
                        <circle cx={cx} cy={cyResolved} r={isHovered ? 5 : 3} fill="#10b981" stroke="#fff" strokeWidth="1.5" />
                      </g>
                    )
                  })}
                  ...
                  {trends.map((t, idx) => {
                    if (idx % Math.ceil(trends.length / 6) === 0 || idx === trends.length - 1) {
                      return (
                        <text key={idx} x={getX(idx)} y={chartH - 4} textAnchor="middle" fontSize="10" fill="#94a3b8">
                          {t.date.slice(5)}
                        </text>
                      )
                    }
                ...
                {hoveredTrendIndex !== null && trends[hoveredTrendIndex] && (
                  ...
                  >
                    ...
                    <div><span style={{ color: '#3b82f6' }}>● Created:</span> {trends[hoveredTrendIndex].created_count}</div>
                    <div><span style={{ color: '#10b981' }}>● Resolved:</span> {trends[hoveredTrendIndex].resolved_count}</div>
                  </div>
                )}
              </div>
            )}
              ...
              <>
                <div className="donut-graphic">
                  <svg viewBox="0 0 170 170" width="140" height="140">
                    ...
                    <text x="85" y="80" textAnchor="middle" fontSize="18" fontWeight="700" fill="#0f172a">
                      {totalDefects}
                    </text>
              ...
              <>
                <div className="donut-graphic">
                  <svg viewBox="0 0 170 170" width="140" height="140">
                    ...
                    <text x="85" y="96" textAnchor="middle" fontSize="10" fill="#ef4444" fontWeight="600">
                      Crit Open
                    </text>
                  </svg>
                </div>
                ...
                {categories.map((cat, idx) => (
                  <div key={cat.name} className="category-row">
                    <div className="category-row-meta">
                      ...
                      <span className="category-val">{cat.count} ({cat.percentage}%)</span>
                    </div>
      ...
      {/* ── Row 3: Developer Workload & Resolution Metrics ── */}
      <section className="analytics-grid-two">
        {/* Developer Workload */}
        <div className="analytics-panel">
          <div className="panel-header">
            <div>
              <h3>Developer Workload & Assignment</h3>
          ...
          <div className="workload-body">
            {workloads.length === 0 ? (
              <div className="analytics-empty">
                ...
                <p>No developers or assigned defects found.</p>
              </div>
            ) : (
                    ...
                    <div key={dev.developer_id} className="workload-item">
                      <div className="workload-meta">
                        <div className="workload-user">
                          <div className="workload-avatar">
                            {dev.developer_name ? dev.developer_name.charAt(0).toUpperCase() : 'D'}
                          </div>
                          <div>
                        ...
                        <div className="workload-counts">
                          <span className="badge-workload active" title="Active (Open + In Progress)">
                            {activeCount} Active
                          </span>
                          <span className="badge-workload resolved" title="Resolved / Closed">
                      ...
                      <div className="workload-progress-bar">
                        {dev.total_assigned > 0 ? (
                          <>
                            <div
                              style={{ width: `${(dev.open / dev.total_assigned) * 100}%`, background: '#3b82f6' }}
                              title={`Open: ${dev.open}`}
                            />
                            <div
                              style={{ width: `${(dev.in_progress / dev.total_assigned) * 100}%`, background: '#8b5cf6' }}
                              title={`In Progress: ${dev.in_progress}`}
                            />
                            <div
                              style={{ width: `${(dev.resolved / dev.total_assigned) * 100}%`, background: '#10b981' }}
                              title={`Resolved: ${dev.resolved}`}
                            />
                            <div
                              style={{ width: `${(dev.closed / dev.total_assigned) * 100}%`, background: '#64748b' }}
        ...
        <div className="analytics-panel">
          ...
          <div className="resolution-panel-body">
            <div className="res-stat-card">
              <div className="res-stat-icon"><i className="bi bi-clock-history" /></div>
              <div>
                <span className="res-stat-label">Average Resolution Time</span>
                <span className="res-stat-val">{resolutionMetrics?.formatted || 'No data'}</span>
            ...
            <div className="res-metrics-grid">
              <div className="res-sub-card">
                ...
                <strong className="res-sub-val">{resolutionMetrics?.sample_size ?? 0} defects</strong>
              ...
              <div className="res-sub-card">
                ...
                <strong className="res-sub-val">
                  {resolutionMetrics?.min_hours !== null && resolutionMetrics?.min_hours !== undefined
              ...
              <div className="res-sub-card">
                ...
                <strong className="res-sub-val">
                  {resolutionMetrics?.max_hours !== null && resolutionMetrics?.max_hours !== undefined
            ...
            <div className="res-doc-note">
              ...
              <span>
                Timestamp fidelity: Measured from <code>created_at</code> to verified <code>updated_at</code> when transitioned to <em>Resolved</em> or <em>Closed</em> status.
              </span>
            </div>
          </div>
        </div>
